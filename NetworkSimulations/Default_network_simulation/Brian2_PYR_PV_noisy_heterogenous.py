...
#############################################################################
#Set parameters on PYR cells one by one to make the population heterogeneous
...
#Values obtained at default parameter values (replaced with cmd arguments in NSG versions)
SFA_target = 0.46 
rheo_target = 4.0
TC_target = -5.0
...
#PYR-PYR
synModel_pyr_pyr = '''
ds0/dt=-(s0-s_inf_pyr_pyr)/(tau_s_pyr_pyr) :1 (clock-driven)
ds1/dt=-s1*beta_pyr_pyr :1 (clock-driven)
...
#PYR-PV
synModel_pyr_pv = '''
ds0/dt=-(s0-s_inf_pyr_pv)/(tau_s_pyr_pv) :1 (clock-driven)
ds1/dt=-s1*beta_pyr_pv :1 (clock-driven)
s_tot=clip(s0,0,s1) :1
...
#PV-PYR
synModel_pv_pyr = '''
ds0/dt=-(s0-s_inf_pv_pyr)/(tau_s_pv_pyr) :1 (clock-driven)
ds1/dt=-s1*beta_pv_pyr :1 (clock-driven)
s_tot=clip(s0,0,s1) :1
...
#PV-PV
synModel_pv_pv = '''
ds0/dt=-(s0-s_inf_pv_pv)/(tau_s_pv_pv) :1 (clock-driven)
ds1/dt=-s1*beta_pv_pv :1 (clock-driven)
s_tot=clip(s0,0,s1) :1
...
#Transient window in milliseconds
transient = 5000
...
#Katie's stats code - brian2 versions
...
fPSC_averages=open(foldername+"/"+filename+"_PSC_stats.txt","w")
